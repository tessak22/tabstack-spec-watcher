...
Source: https://github.com/nodejs/node/releases
Tracks new Node.js releases — major (breaking), minor (additive), security patches.
"""
...
NODE_URL = "https://github.com/nodejs/node/releases"
...
def diff(previous: dict, current: dict) -> list[SpecChange]:
    ...
    for release in current.get("releases", []):
        ...
        # Find closest previous version to compute semver delta
        ...
        changes.append(SpecChange(
            ...
            severity=severity,
            summary=release.get("highlights", f"Node.js {version} released."),
            affects=["Node.js runtime", "npm ecosystem"],
            stage_before=None,
            stage_after=None,
...
if __name__ == "__main__":
    import json
    client = Tabstack()
    print("Fetching Node.js releases...")
    result = fetch(client)
